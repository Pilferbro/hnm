...
@Service
public class Operate25 implements Inspection1JobStrategy {
    ...
    public Object doHandle(Map<String, Object> p) {
        ...
        List<Map<String, Object>> tradeMaps = allTradeInfoDao.countTradesByTerminalId(params);
        if (tradeMaps != null && tradeMaps.size() > 0) {
            for (Map<String, Object> tradeMap : tradeMaps) {
                ...
                if (compareTo(format, num) > 0) {
                    params.put("terminal_id",tradeMap.get("terminal_id"));
                    List<Map<String, Object>> tradeDetails = allTradeInfoDao.queryTrade(params);
                    tradeMap.put("count",countNum);
                    tradeMap.put("tran_addr",tradeDetails.get(0).get("tran_addr"));
