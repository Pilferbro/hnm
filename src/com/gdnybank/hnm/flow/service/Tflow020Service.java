 ...
 */
@Service
public class Tflow020Service extends TXBaseService {
    private static final Log logger = LogFactory.getLog(Tflow020Service.class);
    ...
    public Object doService(Map<String, Object> env, Map<String, Object> p) {
        ...
        if (userRole == 0) {
            ...
            throw new BusinessException("Tflow020ServiceException", "登陆人员未配置角色");
        }
        //风险处置 - 可以看所有
        if (p.containsKey("relation_id") && ObjectUtil.isNotEmpty(p.get("relation_id"))) {
            userRole = 1;
        }
        if (userRole != 1) {
            p.put("operator", userId);
        }
        ...
        List<Map<String, Object>> result = tApprovalApplyDao.queryForListBycondition(delkong(p));
        ...
        typeRuslt.put("approvalApplyList", result);

        logger.info("Tflow020Service执行完成");
        return typeRuslt;
    }
