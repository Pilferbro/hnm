 ...
 */
@Service
public class Tflow026Service extends TXBaseService{
	private static final Log logger = LogFactory.getLog(Tflow026Service.class);
	...
	public Object doService(Map<String, Object> env, Map<String, Object> p) {
		...
		if (p.get("apply_id") == null || "".equals(p.get("apply_id"))) {
			...
			throw new BusinessException("Tflow026ServiceException", "上送“apply_id”为空");
		}
		List<Map<String, Object>> list = new ArrayList<>();
		List<Map<String, Object>> approvalList = approvalDao.queryForList(BaseUtils.map("apply_id",p.get("apply_id")));
		if(approvalList != null && approvalList.size() >0){
			//根据下一审批角色查询该角色下的用户
			list = sysAccountDao.queryByRole(BaseUtils.map("role_id", approvalList.get(0).get("approval_role")));

		}else{
			logger.error("查无此审批流程");
			throw new BusinessException("Tflow026ServiceException", "查无此审批流程");
		}

		logger.info("Tflow026Service执行完成");
		return list;
	}
