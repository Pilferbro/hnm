...
/**
 * 自动化工具生成数据库操作类
 * 表名:DEP_ACCT_BASIC_TEMP
 * 主键:
 **/
@Repository
public class DepAcctBasicTempDao extends TXBaseDao {
    ...
    public int save(Map<String, Object> p) {
        ...
        String sql = Sql.create("insert into DEP_ACCT_BASIC_TEMP (")
                .append(field("id "))
    ...
    public int deleteByPk(Map<String, Object> p) {
        String sql = Sql.create("delete from DEP_ACCT_BASIC_TEMP where 1=1 ")
                .append(and("id = :id"))
                .toString();
        printSql(sql, p);
        return delete(sql, p);
    ...
    public int updateByPk(Map<String, Object> p) {
        String sql = Sql.create("update DEP_ACCT_BASIC_TEMP set ")
    ...
    public List<Map<String, Object>> queryForList(Map<String, Object> p) {
        String sql = Sql.create("select * from DEP_ACCT_BASIC_TEMP where 1=1")
                .append(and("id = :id", hasKey(p, "id")))
                .append(and("lp_cd = :lp_cd", hasKey(p, "lp_cd")))
                .append(and("acct_id = :acct_id", hasKey(p, "acct_id")))
                .append(and("card_no = :card_no", hasKey(p, "card_no")))
    ...
    public List<Map<String, Object>> queryForListByPage(Map<String, Object> p) {
        Sql sql = Sql.create("select * from DEP_ACCT_BASIC_TEMP where 1=1")
                .append(and("id = :id", hasKey(p, "id")))
                .append(and("lp_cd = :lp_cd", hasKey(p, "lp_cd")))
                .append(and("acct_id = :acct_id", hasKey(p, "acct_id")))
                .append(and("card_no = :card_no", hasKey(p, "card_no")))
    ...
    public long count(Map<String, Object> p) {
        String sql = Sql.create("select count(*) from DEP_ACCT_BASIC_TEMP where 1=1 ")
                .append(and("id = :id", hasKey(p, "id")))
                .append(and("lp_cd = :lp_cd", hasKey(p, "lp_cd")))
                .append(and("acct_id = :acct_id", hasKey(p, "acct_id")))
                .append(and("card_no = :card_no", hasKey(p, "card_no")))
    ...
    public int truncate() {
        return super.update("truncate table DEP_ACCT_BASIC_TEMP", BaseUtils.map());
    }

    public int deleteRepData() {
        String sql = "DELETE FROM DEP_ACCT_BASIC_TEMP T1 WHERE T1.ID IN( SELECT T2.ID FROM(SELECT A.*, " +
                "ROW_NUMBER() OVER(PARTITION BY A.ACCT_ID ORDER BY A.ACCT_ID) RN FROM DEP_ACCT_BASIC_TEMP A WHERE " +
                "SUBSTR(a.acct_grad_flg_bit, 7, 1) in ('7','8') )T2 WHERE T2.RN > 1)";
