 ...
 */
@Service
public class Treport006Service extends TXBaseService{
	private static final Log logger = LogFactory.getLog(Treport006Service.class);
	...
	public Object doService(Map<String, Object> env, Map<String, Object> p) {
		...
		if(ObjectUtil.isEmpty(p.get("query_level"))){
			...
			throw new BusinessException("Treport006ServiceException","query_level为必传字段");
		}
		String query_level = p.get("query_level").toString();

        if(ObjectUtil.isEmpty(p.get("query_org"))){
            logger.error("query_org为必传字段");
            throw new BusinessException("Treport006ServiceException","query_org为必传字段");
        }
        ...
        if(userRoleLevel==0){
            throw new BusinessException("Treport006ServiceException", "登陆人员未配置角色");
        }
		...
		logger.info("Treport006Service执行完成");
		return result;
	}
